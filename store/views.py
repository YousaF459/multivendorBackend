...
class CartAPIView(generics.ListCreateAPIView):
    ...
    def create(self,request,*args,**kwargs):
        ...
        if user_id not in ["undefined", "null", None, ""]:
        ...
        if cart:
            ...
            cart.tax_fee=int(qty) * Decimal(taxRate)
            cart.color=color
            cart.size=size
            ...
            return Response({"msg":"New Cart Created"},status=status.HTTP_201_CREATED)
...
class CartDetailView(generics.RetrieveAPIView):
    ...
    def get(self,request,*args,**kwargs):
        ...
        for cartitem in queryset:
            total_shipping+=float(cartitem.shipping_amount)
            total_tax+=float(cartitem.tax_fee)
            total_servicefee+=float(cartitem.service_fee)
            total_subtotal+=float(cartitem.sub_total)
            total_total+=float(cartitem.total)

        data={
            "shipping":total_shipping,
            "tax":total_tax,
            "service_fee":total_servicefee,
...
class CartItemDeleteApiView(generics.DestroyAPIView):
    ...
    def get_object(self):
        ...
        if user_id:
            user=User.objects.get(id=user_id)
            cart=Cart.objects.get(id=item_id,user=user_id,cart_id=cart_id)
        else:
...
class CreateOrderApiView(generics.CreateAPIView):
    ...
    def create(self,request,*args, **kwargs):
        ...
        return Response({"message":"Order Craeted Successfully","order_id":order.oid},status=status.HTTP_201_CREATED)
    ...
    def create(self, request, *args, **kwargs):
        ...
        if coupon:
            ...
            if order_items:
                ...
                return Response({"message": "Coupon Already Activated", "icon": "error"}, status=status.HTTP_200_OK)
        else:
            return Response({"message": "Order Item Not Exist", "icon": "error"}, status=status.HTTP_200_OK)
...
class StripeCheckoutAPIView(generics.CreateAPIView):
    ...
    def create(self,*args,**kwargs):
        ...
        try:
            ...
            return Response({"error":f"Something Went Wrong With Checkout Session : {str(e)}"})
